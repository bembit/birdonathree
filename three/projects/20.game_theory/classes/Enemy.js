...
import { LootDrop } from './bonus/LootDrop.js';
...
export class Enemy {
  ...
  async load() {
    try {
      ...
      // Start with an "idle" animation if available - not necessary unless complex AI is implemented.
      if (actions['idle']) {
        actions['idle'].play();
  ...
  update(delta, player) {
    ...
    // Simple AI: move toward the player when not too close and attack - distance and attackDistance, lastAttackTime etc.
  ...
  die() {
    ...
    if (this.model && this.model.parent) {
      ...
      // Capture enemy's position.
      const lootPosition = this.model.position.clone();

      // Drop something. 
      // Hardcoded, or could add loot to the game manager?
      new LootDrop(lootPosition, this.model.parent, { type: "health", amount: 20 });
